...
/*this is just to show the difference between the syntax. The boost part is
 ...
 want!*/
		...
		setToggleKey('+');
		setDimentions(ofGetWidth(), ofGetHeight()/2.0);
	}
	...
	virtual void render(){
		...
		ofRect(0, 0, /*ofGetWidth()*/width, /*ofGetHeight()/2.0*/height);
		glColor3f(commandlineColor.r, commandlineColor.g, commandlineColor.b);
		ofRect(0, /*ofGetHeight()/2.0*/height-20, /*ofGetWidth()*/width, 20);
		...
		for(iter = textBuffer.begin(); iter!=textBuffer.end(); iter++){
			...
			switch(str.textType){
				case CTEXT_MESSAGE:
					...
					drawBitmapString(str, 20, /*ofGetHeight()/2.0*/height-30-15*(textBuffer.size()-i));
					break;
				case CTEXT_COPY:
		...
		drawBitmapString(commandLine, 20, /*ofGetHeight()/2.0*/height-6);
