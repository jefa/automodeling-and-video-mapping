...
explicacion del metodo en gral
que la correspondencia, en vision estereo, pasa por corresponder dos puntos 2D en los planos imagen de cada una de las camaras con un punto 3D de la escena (algo asi: el SISTEMA stereo SABE que un punto 3D A correponde con el punto 2D X1 de la camara C1 y tambien al punto 2D X2 de la camara C2).

epipolar contraint and triangulation:
.- primero se resuelve el problema de corresponder los puntos (geom epipolar, matriz fundamental, etc.)
.- luego se calcula, mediante triangulacion, la profundidad Z del punto (distancia focal) 


\subsubsection{Geometria epipolar}
explicar la gemoetria epipolar. mencionar que esta basada en el modelo de camara pinhole.
va al anexo.



Visión computacional estéreo es un método para la obtención de la estructura tridimensional de una escena a partir de dos o mas imagenes. 


No se utiliza iluminacion auxuliar, y la correspondencia se establece una correspondencia  establecida entre dos o más imágenes.
