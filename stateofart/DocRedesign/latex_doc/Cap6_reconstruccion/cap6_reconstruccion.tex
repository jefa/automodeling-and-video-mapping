...
El análisis de imágenes de video en estereo se ha utilizado como método para reconstruir la estructura tridimensional de una escena, este método no utiliza luz auxilar,
por esta razón es un método pasivo. El análisis de las imágenes se realiza por medio de algoritmos que han avanzado en los últimos tiempos logrando reconstrucción de escenas
rápidamente, es por esto que esta técnica es muy utilizada en implementaciones que necesitan respuesta en tiempo real.
La reconstrucción utilizando el modelo de cámara estereo se divide en dos pasos, primero resolver el problema de la correspondencia, para todos los puntos de una imagen
se obtiene el punto correspondiente en la otra imagen y se calcula la desigualdad de cada punto, es decir la distancia en pixels entre los puntos que se corresponden.
y segundo resolver el problema de triangulación, teniendo en cuenta la desigualdad definida en la correspondencia utilizando el método de triangulación se obtiene la distancia 
focal de cada punto a cada una de las cámaras, de esta forma se calcula la profundidad del punto.

Both the above tasks are easier to realize when cameras are in a standard setting, that is with parallel
optical axes and coplanar image planes. In this setting epipolar lines correspond to rows in the frame
bufer and point correspondences are searched over these rows. Obviously the standard setting cannot
be obtained with real cameras, and thus a calibration procedure is needed in order to compute the
parameters that relate the ideal model of the cameras to the real devices. This is required because of two
factors: (i) both the correspondence problem and triangulation make the assumption of an ideal model
of the camera (pinhole model ), that can be very diferent from actual imaging devices; (ii) the relative
position and orientation of the two cameras must be known in order to determine range information.

Existen variedad de algorimos que solucionan el problema de correspondecia y cálculo de desigualdad entre los puntos de las dos imágenes se clasifican según diferencias en las imágenes
se destacan dos estrategias:
\begin{itemize}
...
\subsubsection{Geometria epipolar}
...
nates to scene points. Naively, one would simply need to optimize over the
ı
